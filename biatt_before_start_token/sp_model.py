...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        # self.second_qc_att = BiAttention(config.hidden*2, 1-config.keep_prob)
        self.rnn_2 = EncoderRNN(config.hidden*8, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.self_att = BiAttention(config.hidden*2, 1-config.keep_prob)
        self.linear_2 = nn.Sequential(
                nn.Linear(config.hidden*8, config.hidden),
                nn.ReLU()
            )
        # self.linear_2 = Highway(config.hidden*8, config.hidden)

        self.rnn_sp = EncoderRNN(config.hidden*8, config.hidden, 3, False, True, 1-config.keep_prob, False)
        self.linear_sp = nn.Linear(config.hidden*2, 1)

        self.rnn_start = EncoderRNN(config.hidden*8, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_start = nn.Linear(config.hidden*2, 1)

        self.rnn_end = EncoderRNN(config.hidden*10, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_end = nn.Linear(config.hidden*2, 1)

        self.rnn_type = EncoderRNN(config.hidden*10, config.hidden, 1, False, True, 1-config.keep_prob, False)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        # print ("show ques output", ques_output.size())
        ...
        # print ("show context lens", context_lens.size())
...
class Highway(nn.Module):
    ...
    def forward(self, input):
        x_projection_relu = self.linear_1(input)
        x_gate_sigmoid = self.linear_2(x_projection_relu)
        x_highway = (x_gate_sigmoid * x_projection_relu) + (1 - x_gate_sigmoid) * input
        # print ("show highway dims", x_highway.size())
        return x_highway
